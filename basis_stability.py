...
def vmd_centroids(n, W=96, K=8, alpha=1000):
    ...
    spec = importlib.util.spec_from_file_location("dz", "decomp_zoo.py")
    dz = importlib.util.module_from_spec(spec); spec.loader.exec_module(dz)

    df = pd.read_csv("compound_2018_2022.csv", parse_dates=["SETTLEMENTDATE"])
...
def main():
    ...
    ap.add_argument("--out", default="basis_stability.json")
    args = ap.parse_args()
    ...
    for m, kind in ZOO.items():
        p = f"decomp_{m}_K{args.K}_W{args.window}/2018_SA1_price_centroids.npy"
