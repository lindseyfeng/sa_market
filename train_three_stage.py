...
def train_decomposer_only(
    ...
    for x_raw, imfs_true, rrp_next in loader:
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(decomposer.parameters(), 10.0)
        opt.step()
...
# ============================================================
# Main: decomp-only → predictor-only → joint
# ============================================================
def main():
    ...
    ap.add_argument("--pred-epochs", type=int, default=20)
    ...
    ap.add_argument("--out", type=str, default="three_stage_joint.pt")
    ...
    # ========================================================
    # Stage 1: Train decomposer only (frozen predictor)
    # ========================================================
    print("\n====== Stage 1: Train decomposer only (frozen predictor) ======\n")

    opt_dec = torch.optim.Adam(decomposer.parameters(), lr=args.lr)
    ...
    for ep in range(1, args.decomp_epochs + 1):
        ...
        print(
            f"[Decomp {ep:03d}] "
            f"train loss={tr_loss:.4f} | val MSE={va_mse:.4f} MAE={va_mae:.4f}"
        )

    # ========================================================
    # Stage 2: Train predictor only (frozen decomposer)
    # ========================================================
    print("\n====== Stage 2: Train predictor only (frozen decomposer) ======\n")
    ...
    for ep in range(1, args.pred_epochs + 1):
        ...
        print(
            f"[Predictor {ep:03d}] "
            f"train pred={tr_pred:.4f} | val MSE={va_mse:.4f} MAE={va_mae:.4f}"
        )
    ...
    for ep in range(1, args.joint_epochs + 1):
        ...
        va_mse, va_mae = eval_all(decomposer, predictor, va_dl, device)
        print(
            f"[Joint {ep:03d}] train pred={tr_pred:.4f} "
        ...
        if va_mae < best_mae:
            ...
            print(f"  → Saved best checkpoint (val MAE={va_mae:.4f})")
