...
from nvmd_st import STForecaster
...
def load_zoo(arm, year, K, W, channel="SA1_price"):
    ...
    return np.load(os.path.join(f"decomp_{arm}_K{K}_W{W}", f"{year}_{channel}.npy"))
...
def main():
    ...
    ap.add_argument("--panel", default="compound_2018_2022.csv")
    ap.add_argument("--modes", default="vmd_panel_K8_a1000_W96")
    ...
    ap.add_argument("--out", default="three_arms_results.json")
    args = ap.parse_args()
